...
std::vector<RendererDetail> CAESinkFactoryWin::GetRendererDetails()
{
  ...
  if (S_OK == (hr = pEnumerator->GetDefaultAudioEndpoint(eRender, eConsole, pDefaultDevice.GetAddressOf())))
  {
  ...
  {
    CLog::LogF(LOGERROR, "Unable to retrieve default endpoint ({})", CWinError::FormatHRESULT(hr));
  }
  ...
  for (UINT i = 0; i < uiCount; i++)
  {
    ...
    {
      CLog::LogF(LOGERROR, "Unable to retrieve device id ({})", CWinError::FormatHRESULT(hr));
    }
...
struct AEWASAPIDeviceWin32 : public IAEWASAPIDevice
{
  ...
  HRESULT AEWASAPIDeviceWin32::Activate(IAudioClient** ppAudioClient)
  {
    ...
    try
    {
      ...
      {
        CLog::LogF(LOGERROR, "unable to activate IAudioClient ({})", CWinError::FormatHRESULT(hr));
      }
    }
    catch (...) {}
    return hr;
