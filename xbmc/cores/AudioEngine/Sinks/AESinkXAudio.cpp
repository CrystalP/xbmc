...
unsigned int CAESinkXAudio::AddPackets(uint8_t **data, unsigned int frames, unsigned int offset)
{
  ...
  if (!m_running) //first time called, pre-fill buffer then start voice
  {
    ...
    if (FAILED(hr))
    {
      CLog::LogF(LOGERROR, "voice submit buffer failed due to {}", CWinError::FormatXAudio2Error(hr));
    ...
    if (FAILED(hr))
    {
      CLog::LogF(LOGERROR, "voice start failed due to {}", CWinError::FormatXAudio2Error(hr));
  ...
  if (FAILED(hr))
  {
    CLog::LogF(LOGERROR, "submiting buffer failed due to {}", CWinError::FormatXAudio2Error(hr));
    delete xbuffer.pContext;
    return INT_MAX;
  }
...
void CAESinkXAudio::EnumerateDevicesEx(AEDeviceInfoList &deviceInfoList, bool force)
{
  ...
  if (FAILED(hr))
  {
    CLog::LogF(LOGERROR, "failed to activate XAudio for capability testing ({})",
               CWinError::FormatXAudio2Error(hr));
    return;
  }
  ...
  for (int layout = -1; layout <= (int)ARRAYSIZE(layoutsList); layout++)
  {
    ...
    for (int j = 0; j < sizeof(testFormats)/sizeof(sampleFormat); j++)
    {
      ...
      for (int i = 0 ; i < WASAPISampleRateCount; i++)
      {
        ...
        if (FAILED(hr))
          CLog::LogF(LOGERROR, "creating voices failed ({})", CWinError::FormatXAudio2Error(hr));
      }
...
initialize:
  ...
  if (FAILED(hr))
  {
    CLog::LogF(LOGERROR, "Voice start failed : {}", CWinError::FormatXAudio2Error(hr));
  ...
  {
    CLog::LogF(LOGERROR, "GetPerformanceData Failed : {}", CWinError::FormatXAudio2Error(hr));
    return false;
  }
