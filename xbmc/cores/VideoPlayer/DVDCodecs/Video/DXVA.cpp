                              ...
                              HANDLE* pHandle, bool trueShared) const
{
  ...
  for (i = 0; i < count; ++i)
  {
    ...
    if (FAILED(hr))
    {
      CLog::LogF(LOGERROR, "failed creating surfaces. Error {}", CWinError::FormatHRESULT(hr));
      break;
    }
...
void CVideoBufferCopy::Initialize(CDecoder* decoder)
{
  ...
  if (!m_copyRes)
  {
    ...
    {
      CLog::LogF(LOGDEBUG, "unable to get decoder resource. Error {}", CWinError::FormatHRESULT(hr));
      return;
    }

    if (FAILED(hr = pResource.As(&pDecoderTexture)))
    {
      CLog::LogF(LOGDEBUG, "unable to get decoder texture. Error {}", CWinError::FormatHRESULT(hr));
      return;
    }
    ...
    {
      CLog::LogF(LOGDEBUG, "unable to create copy texture. Error {}", CWinError::FormatHRESULT(hr));
      return;
    }
    ...
    {
      CLog::LogF(LOGDEBUG, "unable to get shared handle. Error {}", CWinError::FormatHRESULT(hr));
      return;
    }
