...
ParsedBcp47Tag CBcp47Parser::Parse(std::string str)
{
  ...
  if (tag.m_type == Bcp47TagType::MALFORMED)
...
ParsedBcp47Tag CBcp47Parser::TryParseIso639(const std::string& str)
{
  ...
  if ((str.length() == 2 || str.length() == 3) && StringUtils::IsAsciiLetters(str))
  {
    ParsedBcp47Tag tag;
    tag.m_language = str;
    tag.m_type = Bcp47TagType::REGULAR;
    return tag;
  }
...
ParsedBcp47Tag CBcp47Parser::TryGenericParse(const std::string& str)
{
  assert(std::ranges::none_of(str, [](char c) { return StringUtils::isasciiuppercaseletter(c); }));
...
ParsedBcp47Tag CBcp47Parser::TryParseRegularGrandfathered(const std::string& str)
{
  assert(std::ranges::none_of(str, [](char c) { return StringUtils::isasciiuppercaseletter(c); }));
...
ParsedBcp47Tag CBcp47Parser::TryParseIrregularGrandfathered(const std::string& str)
{
  assert(std::ranges::none_of(str, [](char c) { return StringUtils::isasciiuppercaseletter(c); }));
