...
#include "utils/i18n/TableISO639_1.h"
                                                 ...
                                                 bool checkWin32Locales /*= false*/)
{
  ...
  auto it =
      std::ranges::lower_bound(LanguageCodes, strISO6391Lower, {}, &ISO639::iso639_1);
...
bool CLangCodeExpander::ReverseLookup(const std::string& desc, std::string& code)
{
...

  {
    ...
    auto it = std::ranges::lower_bound(
        ...
        { return StringUtils::CompareNoCase(a, b) < 0; }, &ISO639Entry::name);
...
bool CLangCodeExpander::LookupInISO639Tables(const std::string& code, std::string& desc)
{
  ...
  StringUtils::ToLower(sCode);
  StringUtils::Trim(sCode);
  unsigned long longCode = StringToLongCode(sCode);

  if (sCode.length() == 2)
  {
    auto it = std::ranges::lower_bound(TableISO639_1ByCode, longCode, {}, &ISO639Entry::code);
    if (it != TableISO639_1ByCode.end() && it->code == longCode)
    {
      desc = it->name;
      return true;
    }
    ...
    LANG_LIST list /* = LANG_LIST::DEFAULT */)
{
  ...
  if (format == CLangCodeExpander::ISO_639_2)
    std::transform(g_iso639_2.begin(), g_iso639_2.end(), std::inserter(langMap, langMap.end()),
                   [](const LCENTRY& e) { return std::make_pair(LongCodeToString(e.code), e.name); });
  else
    std::ranges::transform(
        TableISO639_1ByCode, std::inserter(langMap, langMap.end()), [](const ISO639Entry& e)
